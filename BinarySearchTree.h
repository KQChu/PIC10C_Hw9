...
/**	Overloaded copy constructor creates a deep copy of a BinarySearchTree object. Operates by calling on the root the recursive node_copy function which sets up its left/right children and the recursive set_parent function which then updates their node_parent pointers.
...
/**	Move constructor is overloaded because BinarySearchTree utilizes heap memory. Operates by assigning root pointer of rvalue referenceto object being created and setting the original to nullptr. Prints statement "Move performed" to demonstrate when it is used
	@param move_copy is a rvalue reference to the BinarySearchTree object whose data is being given to the newly constructed object (prevents an extra copy from being made)
 */
template<typename T, typename CMP>
...
/**	Creates a deep copy of a TreeNode object starting from the input pointer. Utilizes recursion to cycle through all left and rightchildren to set them in the new TreeNode object
...
/**	Overloaded operator= utilizes swap on copy of BinarySearchTree input to ensure no memory leaks occur. Part of implementing copy swap idiom
...
/**	Inserts a new TreeNode containing input data into BinarySearchTree. Call insert_node(TreeNode<T, CMP>* new_node) TreeNode function with TreeNode pointer created to recursively determine appropriate location
...
/**	Provided that the input value exists within BinarySearchTree, erase(T data) removes TreeNode from BinarySearchTree by rerouting other pointers. For TreeNodes with two children, utilizes largest child of the left subtree. Updates node_parent appropriately
